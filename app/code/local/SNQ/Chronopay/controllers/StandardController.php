<?php
class SNQ_Chronopay_StandardController extends Mage_Core_Controller_Front_Action
{
    /**
    ...
    public function  successAction()
    {
        ...
        try{
            $wsdl = Mage::getStoreConfig('payment/chronopay/wsdl');
            $api = new SNQ_Chronopay_Model_Api($wsdl);
        ...
        if($response['hasErrors'] == 'N' && empty($response['errorMessage'])){
            ...
            if($state == Mage_Sales_Model_Order::STATE_PENDING_PAYMENT){
               $this->_createInvoice($order);
                ...
                $msg = 'Payment completed via Chronopay.';
