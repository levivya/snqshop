...
class Magestore_Shopbybrand_Model_Brand extends Mage_Core_Model_Abstract {
    ...
    /*add by cuong*/
    public function getFeaturedProductIds($brandId){
        $productIdsArray = array();
        $defaultProduct = $this->load($brandId)->getData('product_ids');
        $productIdsArray = explode(",",$defaultProduct);
        $brandProductCollection = Mage::getModel('shopbybrand/brandproducts')->getCollection()
                ->addFieldToFilter('product_id', array('in' => $productIdsArray))
