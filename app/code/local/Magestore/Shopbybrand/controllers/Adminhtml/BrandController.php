        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if (isset($data['url_key'])) {
                ...
                if ($urlRewrite->getId()) {
                    if (!$this->getRequest()->getParam('id')) {
                        ...
                        return;
                    } elseif ($this->getRequest()->getParam('id') && $urlRewrite->getIdentifier() != $urlRewrite->getIdentifier()) {
            ...
            try {
                ...
                $brandModel = Mage::getModel('shopbybrand/brand')
                        ...
                        ->load($model->getId());
                /* edit by cuong */
                $brand = Mage::getModel('shopbybrand/brand')->setStoreId($store)->load($model->getId());
                try {
                    $brandModel->updateUrlKey();
                    if (count($productIds))
                        Mage::helper('shopbybrand')->updateProductsBrand($productIds, $model->getId(), $store);
                    if ($brand->getOptionId() != null) {
                ...
                if ($brand->getOptionId() && $model->getId()) {
                    $brandModel->setOptionId($brand->getOptionId())->save();
                }
                /* end edit by cuong */
