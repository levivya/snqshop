...
class Magestore_Shopbybrand_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function viewAction() {
        ...
        /*add by cuong*/
        $store = Mage::app()->getStore()->getId();
        $template = Mage::getStoreConfig('shopbybrand/brand_detail/brand_detail_template', $store);
        
        // add shop by option
        $block = $this->getLayout()
        ->createBlock('shopbybrand/layer_view')
        ->setTemplate('shopbybrand/layer/view.phtml');
        $this->getLayout()->getBlock('allsidebar')->setChild('shopbybrand-leftnav',$block);
        // end add shop by option
    ...
    public function testAction() {
        $featured = Mage::helper("shopbybrand")->getFeaturedProducts(1);
        foreach ($featured as $product){
            echo $product->getName().' <br>'.$product->getProductUrl().' <br>'.$product->getSmallImage().'<br>';
        }
    }
