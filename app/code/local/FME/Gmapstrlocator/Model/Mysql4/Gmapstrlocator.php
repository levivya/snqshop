...
class FME_Gmapstrlocator_Model_Mysql4_Gmapstrlocator extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    {        
		...
		$links = $object['links'];
		if (isset($links['relatedgmaps'])) {
			$productIds = Mage::helper('adminhtml/js')->decodeGridSerializedInput($links['relatedgmaps']);
			$this->_getWriteAdapter()->delete($this->getTable('gmapstrlocator_products'), $condition);
			
			//Save Related Products
			foreach ($productIds as $_product) {
				$objArray = array();
				$objArray['gmapstrlocator_id'] = $object->getId();
				$objArray['product_id'] = $_product;
				$this->_getWriteAdapter()->insert($this->getTable('gmapstrlocator_products'), $objArray);
			}
		} 
