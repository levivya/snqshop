...
class FME_Gmapstrlocator_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    const XML_GMAP_DISTANCE                     =   'gmapstrlocator/info_popup/map_distance';
    ...
    const XML_GMAP_CURRENT_POS                  =   'gmapstrlocator/manage_search/current_pos';
    
    const XML_GMAP_FILTER_ENABLE                =   'gmapstrlocator/manage_attribute/enable_filters';
    const XML_GMAP_FILTER_HEADING               =   'gmapstrlocator/manage_attribute/filter_text';
    ...
    public function getGMapFilterEnable(){
        
        return Mage::getStoreConfig(self::XML_GMAP_FILTER_ENABLE);
    }
    
    public function getGMapFilterHeading(){
        
        return Mage::getStoreConfig(self::XML_GMAP_FILTER_HEADING);
    }
    
    public function getGMapCustomPos(){
        
        return Mage::getStoreConfig(self::XML_GMAP_CURRENT_POS);
    }
    
    public function getGMapDistance(){
        
        if(Mage::getStoreConfig(self::XML_GMAP_DISTANCE)){
           
           return Mage::getStoreConfig(self::XML_GMAP_DISTANCE); 
        }
    ...
    public function getGMapZoom(){
        ...
        if(Mage::getStoreConfig(self::XML_GMAP_ZOOM) == ''){
            return 8;
        }
    ...
    public function getGMapDetailUrl($detail_iden){
        
        $url = '#';
        
        if($detail_iden){
            
            $url = $this->getGMapListIdentifier().'/'.$detail_iden.$this->getGMapSeoSuffix();
    ...
    function listStoreAttributes($str_arr){
        ...
        if($str_arr){
            ...
            foreach($ids_array as $ids){
                ...
                $html .= '<span>'.$attr['attributes_name'].'</span><br>';
    ...
    function getListOfDistinctTags(){
        
        
        $model = Mage::getModel('gmapstrlocator/attributes');
        $collection = $model->getCollection();
        
        $attribute_data =  $collection->getData();
